...
liquibase --url="jdbc:mariadb://127.0.0.1:3306/$1" --username="root" --password="password" --search-path="$1" snapshot --output-file=../test/$1-mysql-clean.json
sed -i -E '/^ *order: [0-9]+$/d' ../test/$1-mysql-clean.json
